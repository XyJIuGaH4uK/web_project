...
const GamePage = () => {
	...
	useEffect(() => {
		if (!gameData) {
			axios.get(`https://api.rawg.io/api/games/${id}?key=7fc5502620c64a2da2116a770ca355ea`).then((response) => setGameData(response.data))
			axios.get(`https://api.rawg.io/api/games/${id}/screenshots?key=7fc5502620c64a2da2116a770ca355ea`).then((response) => setScreenshots(response.data.results))
		}
	})
						...
						<div className={classes.ratings}>
							<div className={classes.ratingList}>
								{gameData.ratings && gameData.ratings.map(rating => 
									rating && <RatingItem title={rating.title} key={rating.title} percent={rating.percent}/>
								)}
							</div>
